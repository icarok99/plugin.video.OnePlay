...
# -----------------------------
# FUNÇÕES DE TEMPO PADRÃO DO KINGIPTV
# -----------------------------
...
def playlist_pluto():
    ...
    try:
        ...
        if cfscraper:
            ...
            channels = requests.get(url, headers={"User-Agent": USER_AGENT}).json()
        ...
        for channel in channels:
            ...
            if stitched_urls:
                ...
                if stream_url:
                    ...
                    stream_url = stream_url.replace("&appVersion=&",
                                                    "&appVersion=5.9.1-e0b37ef76504d23c6bdc8157813d13333dfa33a3")
            ...
            if current_program:
                ...
                desc += f"[COLOR aquamarine][{local_now.strftime('%H:%M')}] {current_program['title']}[/COLOR]\n({current_program['description']})\n"

            if next_program:
                local_next = next_program["start"].astimezone(timezone(timedelta(hours=-3)))
                desc += f"[COLOR aquamarine][{local_next.strftime('%H:%M')}] {next_program['title']}[/COLOR]\n({next_program['description']})\n"

            # NOME COM PROGRAMA ATUAL
            name_for_kodi = channel_name
            if current_program and current_program.get("title"):
                name_for_kodi = f"{channel_name} - [COLOR aquamarine]{current_program.get('title')}[/COLOR]"

            channels_kodi.append((name_for_kodi, desc, thumb, stream_url))

    except Exception as e:
        log(f"pluto_oneplay: erro geral: {e}")
        raise
