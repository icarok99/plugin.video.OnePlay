...
# def from_str(t):
#     clock = t.split('T')[1].split('.')[0]
#     date = t.split('T')[0]
#     s = '{0}T{1}Z'.format(date,clock)
#     return s
...
def playlist_pluto():
    ...
    url = "https://api.pluto.tv/v2/channels?start={0}&stop={1}".format(from_str, to_str) 
    ...
    if channels_info:
        for n, channel in enumerate(channels_info):
            ...
            if six.PY3:
                ...
                if program_now:
                    channel_name = channel_name + ' - [COLOR aquamarine]' + program_now + '[/COLOR]'
